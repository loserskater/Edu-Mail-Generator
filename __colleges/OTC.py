...
import Student
    ...
    #     'url': 'https://apps.otc.edu/account'
    # }
...
def apply(driver, student):
    ...
    WebDriverWait(driver, 10).until(
        EC.element_to_be_clickable((By.ID, "ssn2"))
    ).send_keys(student.ssn)

    WebDriverWait(driver, 10).until(
        EC.element_to_be_clickable((By.ID, "first"))
    ).send_keys(student.firstName)

    WebDriverWait(driver, 10).until(
        EC.element_to_be_clickable((By.ID, "last"))
    ).send_keys(student.lastName)

    WebDriverWait(driver, 10).until(
        EC.element_to_be_clickable((By.ID, "birthdate"))
    ...
    ).send_keys(student.streetAddress + Keys.ENTER)
